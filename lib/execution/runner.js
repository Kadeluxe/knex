  ...
  async query(obj) {
    ...
    return queryPromise
      ...
      .then((processedResponse) => {
        if (
          obj.method !== 'raw' &&
          obj.transform.single &&
          Array.isArray(processedResponse)
        ) {
          processedResponse = processedResponse[0];
        }

        if (
          obj.transform.assert &&
          (processedResponse === null || processedResponse === undefined)
        ) {
          throw new Error(
            'Query response is empty/null/undefined but query was built using .assert()'
          );
        }
